...
for j in range(num_epochs):
    shuffle(combined)
    print(combined)
    train_X_new = combined[:,:-1]
    train_y_new = combined[:,-1]
    for i in range(num_batches):
        temp_x = train_X_new[i*batch_size:(i+1)*batch_size, :]
        temp_y = train_y_new[i*batch_size:(i+1)*batch_size]
